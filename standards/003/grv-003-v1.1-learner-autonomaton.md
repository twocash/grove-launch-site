...
**Version 1.1** — Public Vision Requirements **Jim Calhoun / The Grove Foundation** — April 2026

*Revision note (v1.0 → v1.1):* Substrate framing promoted from §22 deferral to §1 thesis-level. New §1 callout "Method · Substrate, Not Routing" names the design insight that produced the architecture: accumulated context and judgment live at the sovereign node, the routing layer is the legible mechanism, the substrate is the architectural commitment, variance between nodes is the point. Existing §1 "Method · Composition, Not Invention" callout adds explicit GRV-001 link, names the Autonomaton as composable by design, and frames this specification as one application admitting infinite variations. New §4 paragraph on implementation diversity. New §7 paragraph disambiguating substrate-as-context from substrate-as-compute-environment. New §8 paragraph naming what the Ratchet actually does beneath the tier movement (substrate accumulation as what compounds; tier movement as legible evidence). §12 retitled to "The Learner's Owned Cognitive Substrate"; opening and closing rebalanced to substrate-first framing with institutional grounding as one input among three. §22 deferral paragraph replaced with backreference closing the loop opened at §1; structural-inversion kicker ("put the control where the value is produced") repositioned from mid-paragraph to closing position. No structural changes to pipeline, files, zones, composition interfaces, attestation tiers, co-sign protocol, node taxonomy, terroir argument, capture defense, or institutional preconditions.
...
>
> This specification introduces no new architectural construct. The Learner Autonomaton, the university topology, the co-sign protocol, the attestation tiers — every mechanism specified here is the [Autonomaton pattern (GRV-001)](https://the-grove.ai/standards/001) applied to a learning domain. **The Autonomaton is composable by design.** The atomic unit — three files and a loop — is small enough to compose and sovereign enough to remain coherent under composition. This specification represents one application; the pattern admits infinite variations and refinements over time, across domains. *Composing is the architectural property this document demonstrates.* Other domains — clinical, civic, scientific, professional — are invited to compose against the same primitive on the same terms.

> **Method · Substrate, Not Routing**
>
> Classical computing organizes around deterministic execution against code written by implementers. The Autonomaton organizes differently. Accumulated context and judgment live at the sovereign node, in declarative human-readable form, and the node owner drives the system — including the code-writing tools the system has access to — toward destinations only their experience and the soil they grew in could generate. The five-stage pipeline, the routing layer, the tier landscape are the legible mechanisms by which the substrate is built and consulted. *The substrate itself is the architectural commitment.* The variance between nodes is not a defect; it is the point. The universal architecture enables radical particularity at every node, and that particularity is the source of the value that compounds.
...
## **12\. The Dock: The Learner's Owned Cognitive Substrate**
...
> *Put the control where the value is produced, not where the vendor is incorporated. That is what this architecture does — for the learner, for the institution, for every composition that follows from this one.*
